...
이 파일은 코딩 에이전트(Codex 등)가 이 저장소에서 작업할 때 따라야 할 컨텍스트와 규칙을 정의한다.
기준 기획 문서: Notion "[WEEK16] 기획". 이 파일은 그 문서를 코드 관점으로 옮긴 것이며, 충돌 시 기획 문서가 우선한다.
...
- **데이터 출처**: LEET 기출은 `leet.uwayapply.com` 자료실에서 가져오는 것을 가정하나, 해당 사이트는 자동 접근(크롤링)을 차단한다. 자동 스크래핑을 구현하지 말 것. 데이터는 수동 다운로드 → 파싱 → 검수 파이프라인으로 적재한다.
